...
angular.module("lookAroundApp.controllers", []).controller("ZipCodeFrmCtrl", [
  '$scope',
  '$location',
  '$routeParams',
  ...
  function($scope, $routeParams, $location, googleMap, $http, $filter) {
    ...
    $scope.getLocation = function(details) {
       ...
       * @return {[type]}
       */
      var location = ( details && details.geometry && details.geometry.location ), out = [];
      if(!location) {
    ...
    googleMap.getGeoCoder().geocode({
      ...
      googleMap.placeService.textSearch({
        query: $scope.place, type: $scope.place, location: new googleMap._maps.LatLng(lat, lng), radius: 50
      }, function(data) {
        /*
  ...
  function($scope, $window) {
    ...
    $scope.opts = {
      backdropFade: true, dialogFade: true
    };
  }
]);